...
A successful mutual TLS connection does not remove the requirement to provide a valid OAuth 2.0 access token. Likewise, a valid OAuth 2.0 access token does not remove the requirement to present a valid client TLS certificate during the TLS handshake.
